...
// 	}
	...
	virtual const char_type* getAttributeValue(const char_type* name) const
	{
		...
		if (!attr)
			//return 0;
			// Leon Modified
			return (char_type*)"!!attr_name_does_not_exist!!";
...
private:
	...
	bool setText(char_type* start, char_type* end)
	{
		...
		// Leon modified: changed 'NodeName' to 'NodeData'
		NodeData = replaceSpecialCharacters(s);
	...
	void parseClosingXMLElement()
	{
		...
		if (IsTextElement)
		{
			// the TextElement's name will be based on Element_End's name.
			// the only difference is we have to remove the '/' prefix
